...
try:
    ...
    VIZ_AVAILABLE = True
    logger.info("Agent visualization extension ('agents.extensions.visualization') loaded successfully.")
except ImportError:
    # Define a dummy draw_graph if not available to avoid NameError later
    def draw_graph(*args, **kwargs):
        logger.error("Visualization requested, but 'agentic_team[viz]' extras not installed, Graphviz is missing, or import failed (expected 'agents.extensions.visualization').")
        print("ERROR: Visualization generation failed. Please install 'agentic_team[viz]' (or similar extras) and ensure Graphviz is installed and in PATH.", file=sys.stderr)
        return None
    logger.warning("Could not import 'draw_graph' from 'agents.extensions.visualization'. Visualization will be unavailable. Ensure 'agentic_team[viz]' (or similar) is installed.")
...
async def generate_workflow_visualization() -> None:
    ...
    try:
        ...
        logger.debug(f"Ensured visualization output directory exists: {VISUALIZATION_OUTPUT_DIR}")
    except OSError as e:
        logger.exception(f"Failed to create visualization output directory {VISUALIZATION_OUTPUT_DIR}: {e}")
        print(f"Error: Could not create output directory: {VISUALIZATION_OUTPUT_DIR}", file=sys.stderr)
        return
    ...
    try:
        ...
        graph = draw_graph(agent_to_visualize, filename=str(output_path)) # Pass filename directly
        ...
        if graph: # draw_graph might return the graph object or None on failure
            ...
            logger.info(f"Successfully generated and saved visualization graph to {output_path}")
            print(f"Success: Visualization saved to {output_path}")
            print("Note: The visualization shows static agent definitions (tools/handoffs). Complex orchestration logic (loops, parallel calls) within the code might not be fully represented.")
        else:
            # Error should have been logged by draw_graph (dummy or real)
             logger.error(f"draw_graph function failed to generate visualization for {agent_to_visualize.name}.")
             print(f"Error: Failed to generate visualization for {agent_to_visualize.name}. Check logs and Graphviz installation.", file=sys.stderr)

    except Exception as e:
        logger.exception(f"An unexpected error occurred during visualization generation: {e}")
        print(f"An unexpected error occurred during visualization: {type(e).__name__}: {e}", file=sys.stderr)