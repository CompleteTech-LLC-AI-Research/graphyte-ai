...
from ..config import EVENT_TYPE_MODEL, EVENT_TYPE_OUTPUT_DIR, EVENT_TYPE_OUTPUT_FILENAME # Import new config vars
from ..schemas import EventSchema, SubDomainSchema, TopicSchema # Import new output schema
...
async def identify_event_types(
    ...
    logger.info(f"--- Running Step 4c: Event Type ID (Agent: {event_type_identifier_agent.name}) ---")
    ...
    step4c_run_config = RunConfig(trace_metadata={k: str(v) for k, v in step4c_metadata_for_trace.items()})
    ...
    try:
        ...
        if step4c_result:
            ...
            if isinstance(potential_output_step4c, EventSchema):
                ...
                logger.info("Successfully extracted EventSchema from step4c_result.final_output.")
            elif isinstance(potential_output_step4c, dict):
                try:
                    event_data = EventSchema.model_validate(potential_output_step4c)
                    logger.info("Successfully validated EventSchema from step4c_result.final_output dict.")
                except ValidationError as e:
                    logger.warning(f"Step 4c dict output failed EventSchema validation: {e}")
            else:
                logger.warning(f"Step 4c final_output was not EventSchema or dict (type: {type(potential_output_step4c)}).")

            if event_data and event_data.identified_events:
                # Ensure context fields match
                if event_data.primary_domain != primary_domain:
                    logger.warning(f"Primary domain mismatch in Step 4c output ('{event_data.primary_domain}'). Overwriting with Step 1's ('{primary_domain}').")
                    event_data.primary_domain = primary_domain
                if set(event_data.analyzed_sub_domains) != set(sd.sub_domain for sd in sub_domain_data.identified_sub_domains):
                ...
                event_type_output_content = {
                    ...
                    "identified_events": [item.model_dump() for item in event_data.identified_events],
                ...
                save_result_step4c = direct_save_json_output(
                    EVENT_TYPE_OUTPUT_DIR, EVENT_TYPE_OUTPUT_FILENAME, event_type_output_content, overall_trace_id
                )
                ...
                logger.error("Step 4c FAILED: Could not extract valid EventSchema output.")
        ...
        logger.exception(f"Validation or Type error during Step 4c agent run. Error: {e}", extra={"trace_id": overall_trace_id or 'N/A'})
        print(f"\nError: A data validation or type issue occurred during Step 4c.")
        print(f"Error details: {e}")
        event_data = None
    except Exception as e:
        logger.exception("An unexpected error occurred during Step 4c.", extra={"trace_id": overall_trace_id or 'N/A'})
