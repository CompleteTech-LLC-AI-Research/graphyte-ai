...
from ..config import RELATIONSHIP_MODEL, RELATIONSHIP_OUTPUT_DIR, RELATIONSHIP_OUTPUT_FILENAME
from ..schemas import (
    RelationshipSchema, SingleEntityTypeRelationshipSchema, RelationshipDetail,
    SubDomainSchema, TopicSchema, EntityTypeSchema
)
...
async def identify_relationship_types(
    ...
    entity_types_list_for_step5 = sorted(list(set(
        item.entity_type.strip() for item in entity_data.identified_entities if item.entity_type and item.entity_type.strip()
    )))

    if not entity_types_list_for_step5:
        logger.warning("Step 4a identified an entity list, but it's empty after filtering/stripping. Skipping Step 5.")
        print("\nStep 4a completed, but no specific non-empty entity types were identified. Cannot proceed to relationship analysis.")
        return None
        
    logger.info(f"--- Starting Step 5: PARALLEL Relationship ID (Agent: {relationship_type_identifier_agent.name}) for {len(entity_types_list_for_step5)} Entity Type(s) ---")
    print(f"\n--- Running Step 5: PARALLEL Relationship ID using model: {RELATIONSHIP_MODEL} ---")

    relationship_tasks = []
    entity_types_being_processed = [] # Track which entity type corresponds to which task/result
    ...
    for index, current_entity_type in enumerate(entity_types_list_for_step5):
        logger.debug(f"Preparing task for Step 5 ({index+1}/{len(entity_types_list_for_step5)}): Entity Type Focus '{current_entity_type}'")

        display_entity_type = (current_entity_type[:25] + '...') if len(current_entity_type) > 28 else current_entity_type
        step5_iter_metadata_for_trace = {
            ...
            "context_topic_count": str(sum(len(t.identified_topics) for t in topic_data.sub_domain_topic_map)),
            "context_entity_type_count": str(len(entity_data.identified_entities)),
        }
        step5_iter_run_config = RunConfig(trace_metadata={k: str(v) for k, v in step5_iter_metadata_for_trace.items()})
    ...
    if not relationship_tasks: # Should not happen if entity_types_list_for_step5 was populated, but safeguard
        logger.warning("No relationship identification tasks were prepared in Step 5. Skipping.")
        print("No relationship identification tasks prepared. Skipping Step 5 execution.")
        return None
        
    logger.info(f"Launching {len(relationship_tasks)} relationship identification tasks in parallel...")
    print(f"Running relationship identification for {len(relationship_tasks)} entity types concurrently...")
    step5_results_list = await asyncio.gather(*relationship_tasks, return_exceptions=True)
    ...
    for index, step5_iter_result_or_exc in enumerate(step5_results_list):
        ...
        try:
            ...
            if isinstance(step5_iter_result_or_exc, Exception):
                logger.error(f"Step 5 task for focus '{current_entity_type}' failed with exception: {step5_iter_result_or_exc}", exc_info=step5_iter_result_or_exc)
                print(f"  - Error processing relationships for focus '{current_entity_type}': {type(step5_iter_result_or_exc).__name__}: {step5_iter_result_or_exc}")
                continue
            ...
            if step5_iter_result:
                ...
                single_relation_data: Optional[SingleEntityTypeRelationshipSchema] = None

                if isinstance(potential_output_iter, SingleEntityTypeRelationshipSchema):
                    single_relation_data = potential_output_iter
                    logger.info(f"Successfully extracted SingleEntityTypeRelationshipSchema for focus '{current_entity_type}'.")
                elif isinstance(potential_output_iter, dict):
                    try:
                        single_relation_data = SingleEntityTypeRelationshipSchema.model_validate(potential_output_iter)
                        logger.info(f"Successfully validated SingleEntityTypeRelationshipSchema from dict for focus '{current_entity_type}'.")
                    except ValidationError as e:
                        logger.warning(f"Dict output for focus '{current_entity_type}' failed SingleEntityTypeRelationshipSchema validation: {e}")
                else:
                    logger.warning(f"Output for focus '{current_entity_type}' was not SingleEntityTypeRelationshipSchema or dict (type: {type(potential_output_iter)}). Raw: {potential_output_iter}")

                if single_relation_data:
                    # Ensure focus type matches
                    if single_relation_data.entity_type_focus.strip().upper() != current_entity_type.strip().upper():
                         logger.warning(f"Entity type focus mismatch in output for '{current_entity_type}'. Output had '{single_relation_data.entity_type_focus}'. Correcting.")
                    ...
                    logger.info(f"Step 5 Result for focus '{current_entity_type}': Found {len(relation_details)} relationships.")
                    ...
                    aggregated_relationship_results.append(single_relation_data)
                     ...
                     logger.warning(f"Could not extract valid relationship data for focus '{current_entity_type}'. Raw output: {potential_output_iter}")
                     print(f"  - Warning: Failed to get structured relationships for focus '{current_entity_type}'.")
            else:
                logger.error(f"Step 5 task for focus '{current_entity_type}' returned no result object.")
                print(f"  - Error: Failed to get result object for relationship focus '{current_entity_type}'.")

        except (ValidationError, TypeError) as e:
            logger.exception(f"Validation or Type error processing relationship result for focus '{current_entity_type}'. Error: {e}", extra={"trace_id": overall_trace_id or 'N/A'})
            print(f"\nError: A data validation or type issue occurred processing relationship result for focus '{current_entity_type}'.")
            print(f"Error details: {e}")
        except Exception as e:
            logger.exception(f"An unexpected error occurred processing relationship result for focus '{current_entity_type}'.", extra={"trace_id": overall_trace_id or 'N/A'})
    ...
    if not aggregated_relationship_results:
        logger.warning("Step 5 (Parallel Relationship ID) completed, but no relationship results were successfully aggregated. Final relationship file not saved.")
        print("\nStep 5 (Parallel) completed, but no relationship results were successfully aggregated. Final relationship file not saved.")
        return None
        
    logger.info("Aggregating relationship results from parallel runs and saving final output.")
    ...
    relationship_data = RelationshipSchema(
        ...
        analyzed_sub_domains=[sd.sub_domain for sd in sub_domain_data.identified_sub_domains], # Use list from step 2
    ...
    logger.info(f"Final Aggregated Relationships (Structured):\n{relationship_data.model_dump_json(indent=2)}")
    ...
    relationship_output_content = {
        ...
        "entity_relationships_map": [item.model_dump() for item in relationship_data.entity_relationships_map],
        ...
        "analysis_details": {
            ...
            "entity_types_successfully_processed_count": len(aggregated_relationship_results),
    ...
    save_result_step5_final = direct_save_json_output(
        RELATIONSHIP_OUTPUT_DIR, RELATIONSHIP_OUTPUT_FILENAME, relationship_output_content, overall_trace_id
    )
    print(f"\nSaving final aggregated relationship output file...")
    print(f"  - {save_result_step5_final}")
    logger.info(f"Result of saving final aggregated relationship output: {save_result_step5_final}")
