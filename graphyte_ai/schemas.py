...
class EntityTypeIdentifierSchema(BaseModel):
    """Unscored output schema for entity type identification (Step 4a)."""
    ...
    identified_entities: List[EntityTypeIdentifierDetail] = Field(
        description="Entity types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the entity type analysis.",
    )


class OntologyTypeIdentifierDetail(BaseModel):
    """Represents an ontology type or concept without scores."""
...
class OntologyTypeIdentifierSchema(BaseModel):
    """Unscored output schema for ontology type identification (Step 4b)."""
    ...
    identified_ontology_types: List[OntologyTypeIdentifierDetail] = Field(
        description="Ontology types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the ontology type analysis.",
    )


class EventTypeIdentifierDetail(BaseModel):
    """Represents an event type without scores."""
...
class EventTypeIdentifierSchema(BaseModel):
    """Unscored output schema for event type identification (Step 4c)."""
    ...
    identified_events: List[EventTypeIdentifierDetail] = Field(
        description="Event types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the event type analysis.",
    )


class StatementTypeIdentifierDetail(BaseModel):
    """Represents a statement type without scores."""
...
class StatementTypeIdentifierSchema(BaseModel):
    """Unscored output schema for statement type identification (Step 4d)."""
    ...
    identified_statements: List[StatementTypeIdentifierDetail] = Field(
        description="Statement types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the statement type analysis.",
    )


class EvidenceTypeIdentifierDetail(BaseModel):
    """Represents an evidence type without scores."""
...
class EvidenceTypeIdentifierSchema(BaseModel):
    """Unscored output schema for evidence type identification (Step 4e)."""
    ...
    identified_evidence: List[EvidenceTypeIdentifierDetail] = Field(
        description="Evidence types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the evidence type analysis.",
    )


class MeasurementTypeIdentifierDetail(BaseModel):
    """Represents a measurement type without scores."""

    measurement_type: str = Field(
        description=(
            "The classified type of measurement identified (e.g., Financial Metric, Physical Quantity, Performance Indicator, Survey Result, Count, Ratio, Percentage, Score)."
        )
    )


class MeasurementTypeIdentifierSchema(BaseModel):
    """Unscored output schema for measurement type identification (Step 4f)."""
    ...
    identified_measurements: List[MeasurementTypeIdentifierDetail] = Field(
        description="Measurement types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the measurement type analysis.",
    )


class ModalityTypeIdentifierDetail(BaseModel):
    """Represents a modality type without scores."""
...
class ModalityTypeIdentifierSchema(BaseModel):
    """Unscored output schema for modality type identification (Step 4g)."""
    ...
    identified_modalities: List[ModalityTypeIdentifierDetail] = Field(
        description="Modality types identified within the text relevant to the provided context."
    )
    analysis_summary: Optional[str] = Field(
        None,
        description="Optional notes summarizing the modality type analysis.",
    )
