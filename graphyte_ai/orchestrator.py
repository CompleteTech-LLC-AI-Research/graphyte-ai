...
async def run_combined_workflow(content: str) -> None:
    ...
    domain_only_data: Optional[DomainSchema] = None
        ...
        with trace(
            ...
            if domain_only_data:
                conf_data, rel_data, clar_data = await run_parallel_scoring(
                    domain_only_data.domain, content
                )

                payload = {
                    "domain": domain_only_data.domain,
                    "confidence_score": (
                ...
                domain_output_content = domain_data.model_dump()
                ...
                save_result_domain = direct_save_json_output(
                    DOMAIN_OUTPUT_DIR,
                    DOMAIN_OUTPUT_FILENAME,
                    domain_output_content,
                    overall_trace_id,
                )
                print(f"  - {save_result_domain}")
                logger.info(
                    "Result of saving scored domain output: %s", save_result_domain
                )
