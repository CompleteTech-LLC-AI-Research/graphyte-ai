...
async def run_combined_workflow(content: str) -> None:
    """Runs domain, sub-domain, topic, entity, ontology, event, statement,
    evidence, measurement, modality, measurement instance, and relationship
    identification within a single trace."""
    ...
    try:
        ...
        with trace(overall_trace_metadata["workflow_name"], metadata=overall_trace_metadata) as overall_span:
            ...
            # === Step 5: Extract Measurement Instances ===
            logger.info("--- Starting Step 5: Measurement Instance Extraction ---")
            print("\n--- Starting Step 5: Measurement Instance Extraction ---")
            measurement_instance_data = await extract_measurement_instances(
                content,
                primary_domain,
                sub_domain_data,
                topic_data,
            ...
            # Note: This step currently only uses entity_data. If relationships involving other types (events, statements, etc.)
            # are needed, this step would require modification to accept and use that data.
            ...
            logger.info(f"Step 5 (Measurement Instances) Result: {'Success' if measurement_instance_data else 'Failed/Skipped'}")
